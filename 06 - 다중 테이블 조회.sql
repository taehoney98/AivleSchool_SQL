...
-- 조인 #6
SELECT v.emp_id, e.emp_name, d.dept_name, u.unit_name, v.begin_date, v.reason, v.duration
	FROM vacation AS v
    JOIN employee AS e on v.emp_id = e.emp_id
    JOIN department AS d on e.dept_id = d.dept_id
...
-- 조인 #7
SELECT e.emp_name, e.emp_id, e.dept_id, e.hire_date, v.begin_date, v.reason
	FROM employee AS e
...
-- 조인 #8
SELECT e.emp_name, e.emp_id, e.dept_id,d.dept_name, e.hire_date, v.begin_date, v.reason
	FROM employee AS e
    JOIN vacation AS v ON e.emp_id = v.emp_id
    JOIN department AS d ON e.dept_id = d.dept_id
    WHERE e.retire_date IS NULL;
...
-- 조인 #9
SELECT  dept_name, dept_id, unit_name
	FROM department AS d
    LEFT OUTER JOIN unit AS u ON d.unit_id = u.unit_id;
    
SELECT  dept_name, dept_id, unit_name
	FROM department AS d
    RIGHT JOIN unit AS u ON d.unit_id = u.unit_id; -- outer 생략가능
...
2) LEFT OUTER JOIN -> LEFT JOIN
    ...
    FULL OUTER JOIN -> FULL JOIN buy MySQL XA
3) CROSS JOIN: ON 절이 없음ALTER


*/
-- 조인 # 100
SELECT e.emp_name,e.emp_id, e.dept_id ,v.begin_date, v.reason, v.duration
	FROM employee AS e
